...
- 2026-09-02 V2.1（按确认意见调整+物流费用功能已实现）：
  1. 费用表/实体更名为 `t_wms_logistics_fee` / `LogisticsFee`
  2. **不在** `t_wms_allocate`/`t_wms_shipment` 增加单据类型、调拨分类字段，改为执行业务时由 `LingxingDocTypeUtil` 按始发仓/目的仓实时判定
  3. 领星单据类型并入现有 `ExtDataTypeEnums`（新增 ALLOCATION_ORDER/FBA_SHIPMENT_ORDER/STOCKUP_ORDER），不新建枚举
  4. 单据类型判定封装为工具类 `LingxingDocTypeUtil`（非 Service）
  5. 物流费用功能已实现并通过编译/类型检查：后端（费用表、详情/保存/分摊预览接口、发货前费用校验、列表费用列数据填充）+ 前端（费用弹框、列表预估/实际物流费列、物流费用按钮、查看弹框费用页签、发货0费用二次确认）
  6. 写同步任务、推送领星部分本阶段未实现（后续阶段）

- 2026-09-02 V2.2（物流费用功能按评审意见修订）：
  1. 分摊范围确认：按【物流计划批次】获取发货单列表进行分摊（产品确认，不用SO单号分组；无物流计划时仅计算发货单自身）
  2. 保存逻辑：保存时后端重新查询批次内发货单并按最新数据重算分摊（不信任前端结果），将分摊结果写入批次内全部发货单
